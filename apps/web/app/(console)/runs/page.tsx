...
export default function RunsPage() {
  ...
  }, [agents.data, form.agent_target_id, form.project_id]);
  useEffect(() => {
  ...
  }, [form.model_channel_id, form.project_id, models.data]);
  useEffect(() => {
  ...
  const submit = (event: React.FormEvent) => { event.preventDefault(); const parsed = runInputSchema.safeParse(form); if (!parsed.success) { const next: Record<string, string> = {}; parsed.error.issues.forEach((issue) => { next[String(issue.path[0])] = issue.message; }); setErrors(next); return; } setErrors({}); create.mutate(parsed.data); };
